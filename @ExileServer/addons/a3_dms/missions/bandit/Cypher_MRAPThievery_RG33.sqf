...
if !(_OK) exitWith
{
	diag_log format ["DMS ERROR :: Called MISSION Cypher_MRAPThievery_RG33.sqf with invalid parameters: %1",_this];
};
...
switch (_difficulty) do
{
	case "difficult":
	{
		_msgStart = ['#FFFF00',"Those bastards stole my MRAP! Go kill them and the RG-33 is yours!"];
		_AICount = (12 + (round (random 4)));
		// Do coin toss calculation for vehicle and message
		if ((round (random 1)) isEqualTo 0) then
		{
			_vehicle = ["rhsusf_rg33_usmc_wd",[(_pos select 0) -30, (_pos select 1) -30]] call DMS_fnc_SpawnNonPersistentVehicle;
			_msgWIN = ['#0080ff',"Thanks for wrecking their day Convict! It seems they blew out the RG-33's locks though."];
		}
		else
		{
			_vehicle = ["rhsusf_rg33_usmc_wd",[(_pos select 0) -30, (_pos select 1) -30],_pinCode] call DMS_fnc_SpawnPersistentVehicle;
			_msgWIN = ['#0080ff',format ["Thanks for wrecking their day Convict! As promised, keep the RG-33, entry code %1...",_pinCode]];
		};
	};
...
_msgLOSE = ['#FF0000',"You punks! You let them get away with my RG-33!"];
