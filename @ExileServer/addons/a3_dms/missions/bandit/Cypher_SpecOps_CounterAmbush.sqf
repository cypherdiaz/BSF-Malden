/*
	...
	Shared Features:
		0 AI Patrol Vehicles
		4 AI Static Weapon Emplacements
		
	Crate Info:
		DMS_BoxWeapons_HighGrade
...
switch (_difficulty) do
{
	case "difficult":
	{
		_msgWIN = ['#0080ff',format ["Shit. They sabotaged the locks! But good job killing those bastards"]];
		_AICount = (8 + (round (random 4)));
		_crate_weapons 		= (4 + (round (random 4)));
		_crate_items 		= (3 + (round (random 5)));
		_crate_backpacks 	= (2 + (round (random 3)));
		_vehicle = [_VehicleClass,[(_pos select 0) -30, (_pos select 1) -30],_pinCode] call DMS_fnc_SpawnNonPersistentVehicle;
	};

	case "hardcore":
	{
		_msgWIN = ['#0080ff',format ["Nice job dealing with that ambush Convicts! Entry code %1...",_pinCode]];
...
_crate_loot_values1 =
[
	[_crate_weapons,DMS_BoxWeapons_HighGrade],			// Weapons
