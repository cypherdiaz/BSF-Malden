...
if !(_OK) exitWith
{
	diag_log format ["DMS ERROR :: Called MISSION Cypher_MRAPThievery_Strider.sqf with invalid parameters: %1",_this];
};
...
switch (_difficulty) do
{
	case "difficult":
	{
		_msgStart = ['#FFFF00',"Those bastards stole my MRAP! Go kill them and the Strider is yours!"];
		...
		if ((round (random 1)) isEqualTo 0) then
		{
			...
			_msgWIN = ['#0080ff',"Thanks for wrecking their day Convict! It seems they blew out the Strider's locks though."];
		}
		else
		{
			_vehicle = ["Exile_Car_Strider",[(_pos select 0) -30, (_pos select 1) -30],_pinCode] call DMS_fnc_SpawnPersistentVehicle;
			_msgWIN = ['#0080ff',format ["Thanks for wrecking their day Convict! As promised, keep the Strider, entry code %1...",_pinCode]];
		};
	};
...
_msgLOSE = ['#FF0000',"You punks! You let them get away with my Strider!"];
