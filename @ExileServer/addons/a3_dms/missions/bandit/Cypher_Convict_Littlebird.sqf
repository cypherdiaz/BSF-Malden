/*
	Convict's Littlebird
	Created by Cypherdiaz for BS-Free Servers
	Based on nedslittlebird_mission.sqf by [CiC]red_ned (http://cic-gaming.co.uk)
	...
	Difficult Features:
		100% Chance for Persistent Vehicle on Difficult
		8-12 AI
...
_msgStart = ['#FFFF00',"Another Convict recently bought a Littlebird! It's nice; want to steal it? The word you are looking for is...yes"];
...
switch (_difficulty) do
{
	case "moderate":
	{
		...
		_crate_backpacks 	= (0 + (round (random 2)));
		...
		if ((round (random 1)) isEqualTo 0) then
		{
			...
			_msgWIN = ['#0080ff',"I forgot to mention another group sabotaged the locks huh? Whoops."];
		}
		else
		{
			_vehicle = [_VehicleClass,[(_pos select 0) -30, (_pos select 1) -30],_pinCode] call DMS_fnc_SpawnPersistentVehicle;
			_msgWIN = ['#0080ff',format ["I forgot to mention another group was also trying to steal it too huh? Entry code %1...",_pinCode]];
		};
	};
	...
	case "hardcore":
	{
		_msgWIN = ['#0080ff',format ["Did I say that was owned by a Convict? I meant a government official. Woops. Entry code %1...",_pinCode]];
...
_crate_loot_values1 =
[
	[_crate_weapons,DMS_BoxWeapons_MidGrade],			// Weapons
	_crate_items,							// Items + selection list
	[_crate_backpacks, ["Steerable_Parachute_F"]			// Backpacks
];
...
_msgLOSE = ['#FF0000',"The attackers flew off. You failed."];

// Define mission name (for map marker and logging)
_missionName = "Convict's Littlebird";
