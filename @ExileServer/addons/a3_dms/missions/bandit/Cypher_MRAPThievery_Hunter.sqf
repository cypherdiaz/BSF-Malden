...
if !(_OK) exitWith
{
	diag_log format ["DMS ERROR :: Called MISSION Cypher_MRAPThievery_Hunter.sqf with invalid parameters: %1",_this];
};
...
switch (_difficulty) do
{
	case "difficult":
	{
		_msgStart = ['#FFFF00',"Those bastards stole my MRAP! Go kill them and the Hunter is yours!"];
		...
		if ((round (random 1)) isEqualTo 0) then
		{
			...
			_msgWIN = ['#0080ff',"Thanks for wrecking their day Convict! It seems they blew out the Hunter's locks though."];
		}
		else
		{
			_vehicle = ["Exile_Car_Hunter",[(_pos select 0) -30, (_pos select 1) -30],_pinCode] call DMS_fnc_SpawnPersistentVehicle;
			_msgWIN = ['#0080ff',format ["Thanks for wrecking their day Convict! As promised, keep the Hunter, entry code %1...",_pinCode]];
		};
	};
...
_msgLOSE = ['#FF0000',"You punks! You let them get away with my Hunter!"];
