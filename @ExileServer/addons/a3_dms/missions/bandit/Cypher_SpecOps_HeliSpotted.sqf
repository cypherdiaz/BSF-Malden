/*
	Spec Ops Heli Spotted
	Created by Cypherdiaz for BS-Free Servers
	Based on nedlittlebird_mission by [CiC]red_ned (http://cic-gaming.co.uk)
	Based on work by Defent and Eraser1	

	Moderate/Difficult/Hardcore Mission	
	Moderate Features:
		M-900 Persistent Vehicle on Difficult
		10-14 AI

	Difficult Features:
		MH-9 Persistent Vehicle on Difficult
		12-16 AI

	Hardcore Features:
		UH-1Y(Unarmed) Persistent Vehicle on Hardcore
		14-18 AI

	Shared Features:
		1 AI Patrol Vehicles
		2 AI Static Weapon Emplacements
		
	Crate Info:
		W:From DMS_BoxAntiAir. 0+R2, 1+R2, 2+R2
		0 Items
		1 Backpack
*/
...
switch (_difficulty) do
{
	case "moderate":
	{
		...
		_msgStart = ['#FFFF00',"A SpecOps unit has landed their scout helicopter for refueling! Go kill them and take it for yourself!"];
		_vehicle = [_VehicleClass,[(_pos select 0) -30, (_pos select 1) -30, 0],_pinCode] call DMS_fnc_SpawnPersistentVehicle;
		_msgWIN = ['#0080ff',format ["Convicts stopped the refueling and took the M-900 Helicopter. Entry code %1...",_pinCode]];
		_crate_weapons 		= (0 + (round (random 2)));
		_crate_items 		= 0;
	...
	case "difficult":
	{
		...
		_msgStart = ['#FFFF00',"A SpecOps unit has landed their scout helicopter for refueling! Go kill them and take it for yourself!"];
		_vehicle = [_VehicleClass,[(_pos select 0) -30, (_pos select 1) -30, 0],_pinCode] call DMS_fnc_SpawnPersistentVehicle;
		_msgWIN = ['#0080ff',format ["Convicts stopped the refueling and took the MH-9 Littlebird Helicopter. Entry code %1...",_pinCode]];
	...
	default
	{
		...
		_msgStart = ['#FFFF00',"A SpecOps unit has landed their transport helicopter for refueling! Go kill them and take it for yourself!"];
		_vehicle = [_VehicleClass,[(_pos select 0) -30, (_pos select 1) -30, 0],_pinCode] call DMS_fnc_SpawnPersistentVehicle;
		_msgWIN = ['#0080ff',format ["Convicts stopped the refueling and took the UH-1Y Transport Helicopter. Entry code %1...",_pinCode]];
		_crate_weapons 		= (1 + (round (random 2)));
		_crate_items 		= 0;
		_crate_backpacks 	= (1 + (round (random 1)));
	};
...
_crate_loot_values1 =
[
	[_crate_weapons,DMS_BoxAntiAir],			// Weapons
